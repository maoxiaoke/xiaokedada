...
`Object.assign` 进行的是**深拷贝**，如果源对象的属性值是一个指向对象的引用，只拷贝引用值。

实际上，也不是一个一般的“深拷贝”，只是一级属性的拷贝，而没有继续递归做下一层拷贝。因此，用来“深拷贝”对象是不可以的。
